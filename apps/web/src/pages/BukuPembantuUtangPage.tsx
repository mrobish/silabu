...
const inputCls =
  'w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition';
...
function exportCsv(data: BukuPembantuRow[]) {
  ...
  a.href = url;
  a.download = 'buku-pembantu-utang.csv';
  a.click();
  ...
  return (
    <div className={`${br} overflow-hidden transition hover:shadow-md`}>
      ...
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-3 w-full px-5 py-4 text-left hover:bg-amber-50/30 transition"
      >
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500/20 to-orange-500/20 text-amber-600 shrink-0">
          <Users size={16} />
        </div>
        ...
        <div className="text-right shrink-0 mr-2">
          ...
          <p className="text-sm font-bold text-amber-700 tabular-nums">{rupiah(row.saldoAkhir)}</p>
        </div>
        {open ? (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                  ...
                  row.transactions.map((t, i) => (
                    <tr key={i} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-5 py-2.5">
                        <span className="font-mono text-xs text-amber-600">{t.noJurnal}</span>
                      </td>
                      ...
                      <td
                        className={
                          'px-5 py-2.5 text-right font-medium tabular-nums ' +
                          (t.kredit > 0 ? 'text-amber-700' : 'text-slate-300')
                        }
                      >
                ...
                <tr className="bg-amber-50/60 border-t-2 border-amber-200">
                  <td colSpan={3} className="px-5 py-3 text-right text-xs font-bold text-amber-700 uppercase">
                    Saldo Akhir
                  </td>
                  ...
                  <td className="px-5 py-3 text-right font-bold text-amber-800 text-base tabular-nums">
...
// ─── Main Component ─────────────────────────────────────────────────────────
export default function BukuPembantuUtangPage() {
  const now = new Date();
  ...
  useEffect(() => {
    fetch('/api/accounting/contacts?tipe=supplier', {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/accounting/buku-pembantu/utang?' + params.toString(), {
        headers: { Authorization: 'Bearer ' + token() },
      });
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3 mb-1">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-amber-500/20 to-orange-500/20 animate-pulse" />
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 mb-1">
        ...
        <div className="flex-1">
          <h2 className="text-lg font-bold text-slate-900">Buku Pembantu Utang</h2>
          <p className="text-xs text-slate-500">
            Subsidiary Ledger — Rincian utang per supplier dengan saldo berjalan
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 hover:border-amber-300 transition whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
          ...
          <div className="sm:col-span-2">
            <button
              ...
              onClick={fetchData}
              disabled={loading}
              className="w-full rounded-2xl bg-gradient-to-r from-amber-600 to-orange-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-amber-500/20 hover:shadow-xl transition disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
            >
      ...
      {!loading && data.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-4 shadow-sm">
            <p className="text-xs font-semibold text-amber-500 uppercase">Total Saldo Akhir</p>
            <p className="mt-1 text-lg font-bold text-amber-700 tabular-nums">{rupiah(totalSaldoAkhir)}</p>
          </div>
      ...
      {!loading && data.length === 0 && !error && (
        <div className="text-center py-16">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Pilih supplier dan rentang tanggal, lalu klik "Tampilkan"
          </p>
        </div>
      )}
      ...
      <p className="text-[10px] text-slate-400 text-right">
        Buku Pembantu Utang · SILABU DIGI
      </p>
            ...
            {data.map(row => (
              <div key={row.contactId}>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {/* Saldo Awal */}
                    <tr className="border-t border-gray-800 border-b border-slate-200 bg-amber-50/30">

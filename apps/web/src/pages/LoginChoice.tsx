    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      <div className="flex items-center justify-center p-4" style={{ minHeight: 'calc(100vh - 56px)' }}>
        <div className="grid w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60 md:grid-cols-2 animate-scale-in">
          ...
          {/* LEFT — branded gradient panel (slides in from left) */}
          <div className="relative hidden md:flex flex-col overflow-hidden animate-slide-in-left" style={{background: 'linear-gradient(to bottom right, #059669, #0891b2, #0e7490)'}}>
            {/* subtle dark overlay for text contrast */}
            <div className="absolute inset-0 bg-slate-900/20" aria-hidden="true" />
            {/* wavy SVG divider — sits at right edge of gradient panel */}
            ...
            <div className="relative z-10 flex h-full flex-col items-center justify-center p-12 text-center text-white">
              ...
              <div className="mt-10 space-y-4 w-full max-w-[220px]">
                <div className="flex items-center gap-3 text-xs text-cyan-50/80">
                  <svg className="h-5 w-5 shrink-0 text-cyan-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  <span className="text-left">CoA lengkap 304 akun standar desa</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-cyan-50/80">
                  <svg className="h-5 w-5 shrink-0 text-cyan-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  <span className="text-left">Jurnal umum + saldo awal + laporan otomatis</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-cyan-50/80">
                  <svg className="h-5 w-5 shrink-0 text-cyan-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  <span className="text-left">Data aman & terenkripsi</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 p-6 sm:p-10 md:p-12 animate-slide-in-right stagger-1">
            ...
            {/* MOBILE — gradient header banner (md:hidden) */}
            <div className="md:hidden -mx-6 sm:-mx-10 -mt-6 sm:-mt-10 mb-8 px-6 py-9 text-center text-white relative overflow-hidden"
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm text-cyan-50/90 mt-1">Pilih metode masuk</p>
              </div>
            </div>

            {/* DESKTOP — form header (hidden on mobile, shown by gradient banner) */}
            <div className="hidden md:block mb-8 text-center md:text-left">
              ...
              <p className="text-sm text-slate-500 mt-1">Pilih metode masuk</p>
            </div>
            ...
            <p className="mt-8 text-center text-sm text-slate-500 stagger-5">
              Belum punya akun?{' '}
              <Link to="/register" className="font-semibold text-cyan-600 hover:underline">Daftar</Link>
            </p>
          </div>

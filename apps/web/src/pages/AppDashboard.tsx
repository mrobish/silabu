...
export default function AppDashboard() {
  ...
  const [page, setPageRaw] = useState<Page>(() => {
    ...
    const valid: Page[] = ['dashboard', 'password', 'langganan', 'profil', 'coa', 'saldo-awal', 'jurnal', 'rekap-jurnal', 'penyesuaian', 'rincian-saldo', 'buku-besar', 'laba-rugi', 'neraca', 'neraca-saldo', 'arus-kas', 'perubahan-modal', 'aset-tetap', 'tutup-buku', 'calk', 'bantuan', 'kontak', 'persediaan', 'buku-pembantu-utang', 'buku-pembantu-piutang', 'buku-pembantu-persediaan', 'transaksi-cepat', 'penjualan'];
    ...
    <div className="min-h-screen bg-slate-50">
              ...
              <>
                ...
                {(openGroups.has('setup') || collapsed) && (
                  <div className={'space-y-0.5 ' + (collapsed ? '' : 'animate-fade-in')}>
                    ...
                    {mi('persediaan', 'Data Barang Persediaan', 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4')}
                  </div>
                )}
                ...
                {(openGroups.has('transaksi') || collapsed) && (
                  <div className={'space-y-0.5 ' + (collapsed ? '' : 'animate-fade-in')}>
                    {mi('penjualan', 'Penjualan (Mini POS)', 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z', { hl: true })}
                  ...
                  <div className={'space-y-0.5 ' + (collapsed ? '' : 'animate-fade-in')}>
                    ...
                    {(openGroups.has('buku-pembantu') || collapsed) && (
                      <div className="space-y-0.5">
                        ...
                        {mi('buku-pembantu-persediaan', 'Kartu Stok Persediaan', 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', { sub: !collapsed })}
                      </div>
                    )}
      ...
      <main className={MAIN_ML + ' min-h-screen flex flex-col transition-all duration-300 ease-in-out'}>
        ...
        <header className="sticky top-0 z-50 h-16 bg-white/80 backdrop-blur-md border-b border-slate-100/60 flex items-center gap-3 px-4 sm:px-6">
          ...
          <h2 className="text-lg font-bold text-slate-900">{page === 'password' ? 'Ubah Password' : page === 'langganan' ? 'Langganan' : page === 'profil' ? 'Profil BUM Desa' : page === 'coa' ? 'Bagan Akun (CoA)' : page === 'saldo-awal' ? 'Setup Saldo Awal' : page === 'jurnal' ? 'Jurnal Umum' : page === 'rekap-jurnal' ? 'Rekap Transaksi Jurnal' : page === 'penyesuaian' ? 'Jurnal Penyesuaian' : page === 'rincian-saldo' ? 'Rincian Saldo Awal' : page === 'buku-besar' ? 'Buku Besar' : page === 'laba-rugi' ? 'Laba Rugi' : page === 'neraca' ? 'Neraca' : page === 'neraca-saldo' ? 'Neraca Saldo' : page === 'arus-kas' ? 'Arus Kas' : page === 'aset-tetap' ? 'Aset & Inventaris' : page === 'tutup-buku' ? 'Tutup Buku Tahunan' : page === 'calk' ? 'CALK' : page === 'bantuan' ? 'Panduan' : page === 'kontak' ? 'Kontak' : page === 'persediaan' ? 'Data Barang Persediaan' : page === 'buku-pembantu-utang' ? 'Buku Pembantu Utang' : page === 'buku-pembantu-piutang' ? 'Buku Pembantu Piutang' : page === 'buku-pembantu-persediaan' ? 'Kartu Stok Persediaan' : page === 'penjualan' ? 'Penjualan (Mini POS)' : 'Dashboard'}</h2>
          <div className="flex-1" />

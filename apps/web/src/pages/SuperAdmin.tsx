    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col transform transition-transform duration-200 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          ...
          <button onClick={() => { setPage('settings'); setSidebarOpen(false); }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition ${page==='settings'?'bg-cyan-50 text-cyan-700 hover:bg-cyan-100':'text-slate-600 hover:bg-slate-50'}`}>
        ...
        <div className="p-3 border-t border-slate-100 space-y-2">
          <button onClick={() => { setPage('password'); setSidebarOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 transition">
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="lg:ml-64 min-h-screen">
        ...
        <header className="sticky top-0 z-30 h-14 bg-white border-b border-slate-100 flex items-center gap-3 px-4">
          ...
          <h2 className="text-base font-bold text-slate-900 truncate">{page === 'password' ? 'Ubah Password' : 'Pengaturan Sistem'}</h2>
        </header>

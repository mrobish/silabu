...
function exportCsv(data: BukuPembantuRow[]) {
  const header = [
    'Item', 'Kode', 'Satuan', 'Qty Awal', 'Harga Satuan', 'Akun', 'Saldo Awal',
  ...
  for (const c of data) {
    rows.push([
      c.nama, c.kode, c.satuan, String(c.qtyAwal), String(c.hargaSatuan),
      `${c.akunKode} - ${c.akunNama}`, String(c.saldoAwal),
  ...
  a.href = url;
  a.download = 'buku-pembantu-persediaan.csv';
  a.click();
  ...
  return (
    <div className={`${br} overflow-hidden transition hover:shadow-md`}>
      ...
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-3 w-full px-5 py-4 text-left hover:bg-emerald-50/30 transition"
      >
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 text-emerald-600 shrink-0">
          <Package size={16} />
        </div>
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-slate-500 mt-0.5">
            {row.kode && (
              <span className="inline-flex items-center gap-1">
                <Tag size={10} /> {row.kode}
              </span>
            )}
            {row.satuan && (
              <span className="inline-flex items-center gap-1">
                <Ruler size={10} /> {row.satuan}
              </span>
            )}
            {row.qtyAwal > 0 && (
              <span className="inline-flex items-center gap-1">
                <Hash size={10} /> Qty Awal: {row.qtyAwal.toLocaleString('id-ID')}
              </span>
            )}
            {row.hargaSatuan > 0 && (
              <span className="inline-flex items-center gap-1">
                {rupiah(row.hargaSatuan)}/unit
              </span>
            )}
            {row.akunKode && (
              <span className="inline-flex items-center gap-1">
        ...
        <div className="text-right shrink-0 mr-2">
          ...
          <p className="text-sm font-bold text-emerald-700 tabular-nums">{rupiah(row.saldoAkhir)}</p>
        </div>
        {open ? (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                  ...
                  row.transactions.map((t, i) => (
                    <tr key={i} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-5 py-2.5">
                        <span className="font-mono text-xs text-emerald-600">{t.noJurnal}</span>
                      </td>
                      ...
                      <td
                        className={
                          'px-5 py-2.5 text-right font-medium tabular-nums ' +
                          (t.kredit > 0 ? 'text-emerald-700' : 'text-slate-300')
                        }
                      >
                ...
                <tr className="bg-emerald-50/60 border-t-2 border-emerald-200">
                  <td colSpan={3} className="px-5 py-3 text-right text-xs font-bold text-emerald-700 uppercase">
                    Saldo Akhir
                  </td>
                  ...
                  <td className="px-5 py-3 text-right font-bold text-emerald-800 text-base tabular-nums">
...
export default function BukuPembantuPersediaanPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/accounting/buku-pembantu/persediaan?' + params.toString(), {
        headers: { Authorization: 'Bearer ' + token() },
      });
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3 mb-1">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 animate-pulse" />
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="flex items-center gap-3 mb-1">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 text-emerald-600">
          <Boxes size={20} />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
          ...
          <div className="sm:col-span-2">
            <button
              ...
              onClick={fetchData}
              disabled={loading}
              className="w-full rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-emerald-500/20 hover:shadow-xl transition disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
            >
      ...
      {!loading && data.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4 shadow-sm">
            <p className="text-xs font-semibold text-emerald-500 uppercase">Total Saldo Akhir</p>
            <p className="mt-1 text-lg font-bold text-emerald-700 tabular-nums">{rupiah(totalSaldoAkhir)}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-semibold text-slate-400 uppercase">Jumlah Item</p>
            <p className="mt-1 text-lg font-bold text-slate-900 tabular-nums">
              {totalItems.toLocaleString('id-ID')} item
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {!loading && data.length === 0 && !error && (
        <div className="text-center py-16">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Pilih item persediaan dan rentang tanggal, lalu klik "Tampilkan"
          </p>
        </div>
      )}
      ...
      <p className="text-[10px] text-slate-400 text-right">
        Buku Pembantu Persediaan · SILABU DIGI
      </p>

      {/* Print Modal */}
      <ReportPrintLayout
        title="BUKU PEMBANTU PERSEDIAAN"
        isOpen={printOpen}
            ...
            {data.map(row => (
              <div key={row.itemId}>
                <div className="mb-2 font-bold text-[12px] border-b border-slate-800 pb-1">
                  {row.nama} ({row.kode}) · {row.satuan}
                  {row.qtyAwal > 0 ? ` · Qty Awal: ${row.qtyAwal.toLocaleString('id-ID')}` : ''}
                  {row.hargaSatuan > 0 ? ` · ${rupiah(row.hargaSatuan)}/unit` : ''}

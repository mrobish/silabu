...
function TutorialSection() {
  ...
  const steps = [
    ...
    },
    {
      id: 'alur',
      icon: '🔄',
      title: 'Alur Pengisian yang Benar',
      content: (
        <div className="space-y-3 text-sm text-slate-700">
          <div className="relative">
            <div className="absolute left-4 top-0 bottom-0 w-0.5 bg-emerald-200" />
            {[
              { step: 1, label: 'Isi Saldo Awal', desc: 'Menu "Setup Saldo Awal" → input semua saldo awal akun (Kas, Bank, Persediaan, Aset, Utang, Modal)', color: 'emerald' },
              { step: 2, label: 'Buka Rincian Saldo', desc: 'Menu ini → pilih tab yang mau diisi', color: 'blue' },
              { step: 3, label: 'Input Rincian per Tab', desc: 'Persediaan → Hutang/Piutang → Aset Tetap → Modal', color: 'purple' },
              { step: 4, label: 'Cek Badge Rekonsiliasi', desc: 'Hijau = cocok ✅, Merah = selisih ❌', color: 'amber' },
            ].map((s, i) => (
              <div key={i} className="relative pl-10 pb-4">
                <div className={`absolute left-2.5 w-3 h-3 rounded-full bg-${s.color}-500 border-2 border-white shadow`} />
    ...
    },
    {
      id: 'aset-tetap',
      icon: '🏗️',
      title: 'Tab 3: Aset Tetap ⚠️ PENTING',
      content: (
        <div className="space-y-4 text-sm text-slate-700">
          ...
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  <tr className="border-b border-emerald-200"><td className="py-1 pr-2">Komputer Admin</td><td className="text-right px-2">Rp 15.000.000</td><td className="text-right px-2">Rp 3.000.000</td><td className="text-right pl-2">Rp 12.000.000</td></tr>
                  <tr><td className="py-1 pr-2">Motor Operasional</td><td className="text-right px-2">Rp 25.000.000</td><td className="text-right px-2">Rp 10.000.000</td><td className="text-right pl-2">Rp 15.000.000</td></tr>
                  <tr className="border-t-2 border-emerald-400 font-bold"><td className="py-1 pr-2 text-right">Total:</td><td className="text-right px-2">Rp 40.000.000</td><td className="text-right px-2">Rp 13.000.000</td><td className="text-right pl-2">Rp 27.000.000</td></tr>
                </tbody>
              </table>
            </div>
            <div className="mt-3 space-y-1 text-xs text-emerald-700">
              <p>→ Di <strong>Saldo Awal</strong>, akun 1.3.03.01 (Peralatan) harus = <strong>Rp 15.000.000</strong> (harga perolehan Komputer)</p>
              <p>→ Di <strong>Saldo Awal</strong>, akun 1.3.02.01 (Kendaraan) harus = <strong>Rp 25.000.000</strong> (harga perolehan Motor)</p>
              <p>→ Di <strong>Saldo Awal</strong>, akun 1.3.07.02 (Akum. Peralatan) harus = <strong>Rp 3.000.000</strong></p>
              <p>→ Di <strong>Saldo Awal</strong>, akun 1.3.07.01 (Akum. Kendaraan) harus = <strong>Rp 10.000.000</strong></p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  <tr className="border-b border-emerald-200"><td className="py-1 pr-2"><span className="px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 text-[10px] font-bold">Pemerintah Desa</span></td><td className="px-2">2024</td><td className="px-2">Dana Desa tahap 1</td><td className="text-right pl-2 font-bold">Rp 50.000.000</td></tr>
                  <tr><td className="py-1 pr-2"><span className="px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 text-[10px] font-bold">Masyarakat</span></td><td className="px-2">2024</td><td className="px-2">Swadaya warga</td><td className="text-right pl-2 font-bold">Rp 5.000.000</td></tr>
                  <tr className="border-t-2 border-emerald-400 font-bold"><td colSpan={3} className="py-1 pr-2 text-right">Total Rincian:</td><td className="text-right pl-2">Rp 55.000.000</td></tr>
                </tbody>
              </table>
            </div>
            <p className="text-xs text-emerald-700 mt-2">→ Di <strong>Saldo Awal</strong>, akun 3.1.01.01 (Modal Desa) harus = <strong>Rp 55.000.000</strong></p>
          </div>
        </div>
      ),
    },
  ...
  return (
    <div className="mt-8 border-t-2 border-slate-200 pt-6">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-cyan-500 flex items-center justify-center text-white text-lg shadow-lg shadow-emerald-500/20">
          📚
        </div>
        <div>
          <h2 className="text-lg font-bold text-slate-900">Panduan Pengisian Rincian Saldo</h2>
          <p className="text-xs text-slate-500">Tutorial step-by-step untuk bendahara BUM Desa</p>
        </div>
      </div>

      <div className="space-y-2">
        {steps.map((step) => (
          <div key={step.id} className="rounded-xl border border-slate-200 overflow-hidden bg-white">
            <button
              onClick={() => toggle(step.id)}
              className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-slate-50 transition-colors"
            >
              <span className="text-xl shrink-0">{step.icon}</span>
              <span className="flex-1 font-semibold text-sm text-slate-900">{step.title}</span>
              <svg className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${openStep === step.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
